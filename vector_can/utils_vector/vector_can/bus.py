...
from . import CanBus, CanMessage
from . import len2dlc, dlc2len
...
class VectorCanBus(CanBus):
    ...
    def _open_port(self, queue_size):
        if self.fd:
            VectorCanApi.xlOpenPort(
                pointer(self.port_handle),
                pointer(self._app_name),
                self.mask,
                pointer(self.permission_mask),
                queue_size,
    ...
    def send(self, msg: CanMessage, timeout: typing.Optional[float] = None):
        self._send_sequence([msg])

    def _send_sequence(self, msgs: typing.Sequence[CanMessage]) -> int:
        ...
        for channel_config in channel_configs:
            if (
                    not channel_config.channelBusCapabilities
                        & v_def.XL_BusCapabilities.XL_BUS_ACTIVE_CAP_CAN.value
            ):
                continue
            LOG.info(
                "Channel index %d: %s",
