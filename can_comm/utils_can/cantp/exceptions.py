"""
    ...
    :synopsis:
        Implementation of ISO15765-2 exceptions.
    :author:
...
class IsoTpError(Exception):
    """
    Parent class for ISO-TP exceptions
    """
...
class InvalidCanDataError(IsoTpError):
    """
    Happens when a CAN message that cannot be decoded as valid First Frame, Consecutive Frame, Single Frame or
    Flow Control PDU is received.
    """
...
class MissingEscapeSequenceError(IsoTpError):
    """
    Happens when a SingleFrame with length (CAN_DL) greater than 8 bytes is received and the length of the payload
    (SF_DL) is encoded in the first byte, which is forbidden by ISO-15765-2
    """
...
class OverFlowError(IsoTpError):
    """
    Happens when the TransportLayer receive a FlowControl PDU with a FlowStatus=Overflow (2). In this event,
